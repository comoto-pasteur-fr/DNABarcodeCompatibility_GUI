 ...
 * Creates the visual association between the sample and the barcode(s) according to chemistry.
 */
class TaggedSample extends HBox {
     ...
     * @param chemistry the chemistry used for the experiment.
     */
    TaggedSample(String[] resultLine, int chemistry){
        ...
        if(result.length == 4) {
            ...
            for (int i = 0; i <= sequenceLength - 1; i++) {// the sequence
                this.getChildren().add(new ColorLabel(sequence[i], chemistry));
            }
            ...
            for (int i = 0; i <= sequenceLength - 1; i++) {// the sequence1
                this.getChildren().add(new ColorLabel(sequence1[i], chemistry));
            }
            this.getChildren().add(new SampleLabel(resultLine[0]));//the sample

            for (int i = 0; i <= sequenceLength - 1; i++) {// the sequence2
                this.getChildren().add(new ColorLabel(sequence2[i], chemistry));
            }
